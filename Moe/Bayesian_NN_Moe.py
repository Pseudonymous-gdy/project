...
class Bayesian_NN_Moe(nn.Module):
    ...
    def _router_moments(self, h: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        var_m = (h*h) @ exp(W_logvar)^T + exp(b_logvar)
        """
        # Ensure dtype alignment with router params for AMP safety
        if h.dtype != self.W_mu.dtype:
            h = h.to(self.W_mu.dtype)
    ...
    def _route_and_mix(self, h: torch.Tensor, gate_scores: torch.Tensor) -> Tuple[torch.Tensor, Dict]:
        ...
        for e in range(E):
            ...
            # Run expert e on its tokens with dtype alignment
            feat_e = h[token_idx]
            # Ensure input dtype matches expert parameters to avoid AMP mismatch
            p_dtype = next(self.experts[e].parameters()).dtype
            if feat_e.dtype != p_dtype:
                feat_e = feat_e.to(p_dtype)
            y_e = self.experts[e](feat_e)                          # [Be,C]
            ...
            # Accumulate with dtype alignment for AMP (bf16/fp16 safety)
    ...
    def forward_warmup(self, x: torch.Tensor, *, return_aux: bool = False):
        ...
        # Align feature dtype with sampled router params for AMP safety
        if h.dtype != W_sample.dtype:
            h = h.to(W_sample.dtype)
